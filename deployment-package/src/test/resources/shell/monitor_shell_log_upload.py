...
## 最大的 timeout 时间，超过这个时间没有日志可以读取，进程一定退出, 2017/4/20 by linyi
MAX_TIMEOUT = 300
    ...
    with open(file_name, 'r') as file:
        ...
        while is_continue:
            ...
            if not line:
                ...
                if (time_count > MAX_TIMEOUT or (time_count >= no_content_timeout  and len(content_arr) ==0)):
                    # print '退出 python'
                    is_continue = False
                    exit(0)
...
if __name__ == '__main__':
    if len(argv) < 6:
        print '参数不正确, 正确参数: [file_name/文件全路径]  [no_content_time_out/空内容超时时间] [type/日志类型:(shell|compile|deploy)] [server_deploy_id/服务器发布id(当为type类型为compile时为项目的module id)] [server_url/接口地址]'
        exit(1)
    ...
    logType = 0
    if type != 'shell' and type != 'compile' and type != 'deploy':
