...
backupModule() {
    ...
    mkdir -p ${BACKUP_DIR}/$backupTime-$deployId
    if [ -d ${MODULE_DIR}/release-current ];then
        hasFile=`ls ${MODULE_DIR}/release-current | wc -l`
        if [ $hasFile  -gt 0 ];then
            cp -rf ${MODULE_DIR}/release-current/* ${BACKUP_DIR}/$backupTime-$deployId
        fi
    fi
    cd ${BACKUP_DIR}
    # 删除备份文件，只保留20个版本
...
decompressModule() {
    mkdir -p ${MODULE_DIR}/release-current
    mkdir -p ${MODULE_DIR}/new-version

    cd ${PROJECT_DIR}
    tar xf ${MODULE_TAR_FILE} -C ${MODULE_DIR}/new-version
    decompressResult=$?
...
rollBackModuleFile() {
    ...
    # 恢复 jar/war
    rm -rf ${MODULE_DIR}/release-current/*
    \cp -rf ${BACKUP_DIR}/$rollBackVersion/* ${MODULE_DIR}/release-current
}
...
rollBack)
      ...
      rollBackModuleFile 1> /dev/null 2>${MODULE_ERR_LOG}
      logErrorInfo
