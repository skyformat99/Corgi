...
ArtPaginator.prototype = {
	...
	// 读取表格有多少列
	getColumns: function() {
		var tr = $("#" + this.options.table + ' tr');
		if (typeof tr == 'undefined' || tr.length == 0) {
			// 没有定义表头
			return 1;
		}
		if (tr.length > 0) {
			return $(tr[0]).children().length;
		}
		return 1;
	},
	...
	load: function(page) {
		...
		*/
		...
		$.ajax({url: this.options.url, type: reqType, data: params, dataType: 'json', success: function(json) {
				if(!json.success) {
					MyDialog.alert('结果错误：' + json.message);
					return;
				}
	...
	showEmptyContent: function() {
		...
		this.getTBody().html('<tr><td colspan="' + columns + '">没有数据</td></tr>');
	},
	...
	redrawPageBar : function(currentPage, json) {
		if (typeof json.totalPage == 'undefined' || json.totalPage === 0) {
			// 没有总页数，只显示prev, next
